...
class ClientHandler(ConnectionHandler):
    ...
    def run(self):
        logger.info(f"Receiving from {self.connection.address}")
        while self.active:
            self.receive()
    ...
    def receive(self):
        try:
            ...
            if message:
                logger.info(f"Received message {message} from {self.connection.address}")
    ...
    def run(self):
        ...
        try:
            while self.active:
                ...
                logger.info(f"Accepted new connection from {connection.address}")
        except ConnectionError:
            self.disconnected = True
...
class Server:
    ...
    def broadcast(self, encoded_message):
        logger.info(f"Broadcasting {encoded_message}")
    ...
    def process_message(self, encoded_message):
        logger.debug(f"New message: {encoded_message}")
    ...
    def __init__(self):
        ...
        self.listener = Listener((socket.gethostname(), config.server_port), self.new_connections)
        self.listener.start()
    ...
    def update_receivers(self):
        for client_handler in self.client_handlers:
            if client_handler.disconnected:
                logger.info(f"{client_handler.connection.address} disconnected")
